...
export const PasseadorFormEditScreen = observer(function PasseadorFormEditScreen() {
  ...
  const { passeadorStore, petStore } = useStores()
  ...
  const [passeadorId, setSelecionarPasseadorId] = useState(0);
  ...
  useEffect(() => {
    ...
    passeador=passeadorStore.getPasseadorPorId(passeadorStore.passeador)
    setSelecionarPasseadorId(passeadorStore.passeador)
    ...
    petRadioGroup.forEach(element => {
      element.selected=(parseInt(element.value)===passeador.petId)
      arrVet.push(element)
    })
    setSelecionarPetId(arrVet)
  }, [])

  function adicionarNovoPasseador() {
    const petSelecionado = petRadioGroup.find((item) => {
      return (item.selected)
    })
    if (petSelecionado) {
    passeadorStore.savePasseador({
      id: passeadorId ,
